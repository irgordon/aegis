...
| Local credential injection | Provide safe local credential handle references | satisfied credential boundary | non-secret credential handle reference |
| Wrapper dispatcher | Find and run a registered wrapper | wrapper name, version, request, authorization, credential context | wrapper execution result or structured failure |
...
| Recovery planner | Convert inspection report into bounded future recovery guidance | inspection report | recovery plan report |
...
| `CredentialInjectionResult` | A safe local credential handle reference, not a secret |
...
Wrappers execute only after the gateway supplies authorization and credential boundary evidence.
